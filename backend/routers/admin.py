...
from backend.services.config import get_admin_password
...
from backend.services.process_status import update_status, reset_status, STATUS, start_job, finish_job, fail_job
import random
...
@router.post("/session")
def validate_admin(_: None = Depends(verify_admin)):
    return {"status": "ok"}
...
def get_stats(_: None = Depends(verify_admin)):
    try:
        df = repo.load_articles_for_stats()
...
def _run_crawl(max_articles: int, categories: list[str] = None, total_articles: int = None):
    ...
    try:
        ...
        if not df_raw.empty:
            articles_count = build_ready_rows_from_naver(df_raw)
        finish_job(articles_count)
    except Exception as e:
        fail_job(str(e))
    finally:
...
@router.post("/crawl")
def start_crawl(req: CrawlRequest, background_tasks: BackgroundTasks, _: None = Depends(verify_admin)):
...
def get_process_status(_: None = Depends(verify_admin)):
    return STATUS
...
@router.post("/analyze")
def start_analyze(background_tasks: BackgroundTasks, _: None = Depends(verify_admin)):
    df = repo.load_articles_without_trust()
    count = len(df)
    if count == 0:
...
def _run_dedupe():
    ...
    try:
        ...
        finish_job(len(to_delete))
    except Exception as e:
        fail_job(str(e))


@router.post("/dedupe")
def start_dedupe(background_tasks: BackgroundTasks, _: None = Depends(verify_admin)):
    """DB 내 중복 기사 일괄 제거"""
    df = repo.load_articles()
    if df.empty:
...
def delete_no_keywords(_: None = Depends(verify_admin)):
    count = repo.delete_articles_without_keywords()
    return {"status": "done", "deleted_count": count}


@router.post("/keywords")
def start_keywords(background_tasks: BackgroundTasks, _: None = Depends(verify_admin)):
...
@router.get("/jobs")
def get_background_jobs(_: None = Depends(verify_admin)):
    """배경 작업 이력 조회 (최근 20건)."""
    return repo.list_background_jobs(limit=20)


@router.get("/performance-metrics")
def get_performance_metrics(hours: int = 24, _: None = Depends(verify_admin)):
    """클라이언트 성능 지표 집계 (경로별 p50/p75/p95 LCP, p75 CLS·INP)."""
    return repo.get_performance_metrics(hours=hours)


@router.get("/dashboard-stats")
def get_dashboard_stats():
    """대시보드 종합 통계 반환 API (비밀번호 체크 생략)"""
