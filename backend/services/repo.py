...
def get_user_info(user_id: str) -> dict | None:
    ...
    cur = con.cursor()
    try:
        cur.execute("SELECT user_id, created_at FROM users WHERE user_id = %s", [user_id])
        row = cur.fetchone()
        if not row:
            return None
...
def get_dashboard_stats() -> dict:
    """파이썬 Pandas 분석 로직 이용한 대시보드 통계 추출"""
    try:
        con = _get_con()
        cur = con.cursor()
        ...
        cur.execute("""
            SELECT
                COUNT(DISTINCT CASE WHEN event_type = 'visit_main' THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'view_article_detail' THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'open_trust_modal' THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'click_feedback' THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'visit_main' AND user_id IS NOT NULL THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'view_article_detail' AND user_id IS NOT NULL THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'open_trust_modal' AND user_id IS NOT NULL THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'click_feedback' AND user_id IS NOT NULL THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'visit_main' AND user_id IS NULL THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'view_article_detail' AND user_id IS NULL THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'open_trust_modal' AND user_id IS NULL THEN session_id END),
                COUNT(DISTINCT CASE WHEN event_type = 'click_feedback' AND user_id IS NULL THEN session_id END)
            FROM event_logs
        """)
        f_row = cur.fetchone()

        # 2. 일자별 활성 세션 수 (에러 방지를 위해 SUBSTRING 사용)
        cur.execute("""
            SELECT SUBSTRING(created_at, 6, 5) as dt, COUNT(DISTINCT session_id)
            FROM event_logs
            WHERE created_at IS NOT NULL AND created_at != ''
            GROUP BY SUBSTRING(created_at, 1, 10), SUBSTRING(created_at, 6, 5)
            ORDER BY SUBSTRING(created_at, 1, 10) ASC
            LIMIT 14
        """)
        das_rows = cur.fetchall()

        # 3. TOP 10 기사 (조회수가 1 이상인 것만)
        cur.execute("""
            SELECT 
                COALESCE(a.title, '알 수 없는 기사(삭제됨)') as title,
                COUNT(CASE WHEN e.event_type = 'view_article_detail' THEN 1 END) as view_cnt,
                COUNT(CASE WHEN e.event_type = 'open_trust_modal' THEN 1 END) as trust_cnt,
                COUNT(CASE WHEN e.event_type = 'click_feedback' THEN 1 END) as feedback_cnt
            FROM event_logs e
            LEFT JOIN articles a ON e.article_id = a.article_id
            WHERE e.article_id IS NOT NULL AND e.article_id != ''
            GROUP BY a.article_id, a.title
            HAVING COUNT(CASE WHEN e.event_type = 'view_article_detail' THEN 1 END) > 0
        ...
        return {
            "funnels": {
                "overall": {"visit_main": f_row[0] or 0, "view_article_detail": f_row[1] or 0, "open_trust_modal": f_row[2] or 0, "click_feedback": f_row[3] or 0},
                "member": {"visit_main": f_row[4] or 0, "view_article_detail": f_row[5] or 0, "open_trust_modal": f_row[6] or 0, "click_feedback": f_row[7] or 0},
                "guest": {"visit_main": f_row[8] or 0, "view_article_detail": f_row[9] or 0, "open_trust_modal": f_row[10] or 0, "click_feedback": f_row[11] or 0}
            },
            "das": [{"date": r[0], "sessions": r[1]} for r in das_rows],
            "top_articles": [{"title": r[0], "views": r[1], "trusts": r[2], "feedbacks": r[3]} for r in top_articles]
        }
    except Exception as e:
        print(f"Dashboard Stats Error: {e}")
        return {"funnels": {}, "das": [], "top_articles": []}
