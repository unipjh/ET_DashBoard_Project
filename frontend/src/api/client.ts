...
export interface FunnelStepData {
  visit_main: number;
  view_article_detail: number;
  open_trust_modal: number;
...
export interface DashboardStats {
  funnels: {
    ...
    member: FunnelStepData;
    guest: FunnelStepData;
  };
  das: { date: string; sessions: number }[];
  top_articles: { title: string; views: number; trusts: number; feedbacks: number }[];
}
